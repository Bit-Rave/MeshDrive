...
async function showMoveFileDialog() {
    ...
    try {
        ...
        try {
            ...
            // Si ça échoue, essayer de récupérer au moins les dossiers de la racine
            try {
        ...
        const hasRoot = allFolders.some(f => f.folder_path === '/');
        if (!hasRoot) {
            allFolders.unshift({
                folder_id: 'root',
                folder_name: '/',
        ...
        function buildFolderTree(folders) {
            ...
            folders.forEach(folder => {
                ...
                if (!folderNode) return; // Sécurité
                
                if (folder.parent_path === '/' || !folder.parent_path) {
                    // Dossier racine (parent est "/" ou vide)
                    rootFolders.push(folderNode);
                } else {
                    // Dossier enfant - chercher le parent
                    const parent = folderMap.get(folder.parent_path);
                    if (parent) {
                        parent.children.push(folderNode);
                    } else {
                        // Parent non trouvé dans la liste (peut-être filtré), traiter comme racine
                        rootFolders.push(folderNode);
                    }
                }
            });
            
            // Trier les enfants de chaque nœud récursivement
        ...
        function renderFolderTree(tree, indent = 0) {
            let html = '';
            tree.forEach(node => {
                // Utiliser des espaces pour l'indentation (4 espaces par niveau)
                const indentStr = '    '.repeat(indent);
                // Afficher le chemin complet avec un slash à la fin
        ...
        // Afficher la modal avec le contenu
        ...
        // Créer un bouton OK personnalisé
...
// Fonction pour exécuter le déplacement
...
async function listAllFolders() {
    try {
        // Utiliser l'endpoint dédié pour récupérer tous les dossiers
        ...
        const hasRoot = validFolders.some(f => f.folder_path === '/');
        if (!hasRoot) {
        ...
        // Retourner au moins la racine si l'API échoue
        return [{
            folder_id: 'root',
            folder_name: '/',
            folder_path: '/',
