...
async function loadFiles(folderPath = null) {
    ...
    try {
        ...
        // Vérifier que la fonction est disponible (window ou global)
        const getContents = window.getFolderContents || getFolderContents;
        if (typeof getContents !== 'function') {
            throw new Error('getFolderContents n\'est pas disponible. Veuillez recharger la page.');
        }
        const contents = await getContents(currentFolderPath);
