...
try:
    from .config import (
        LOG_LEVEL, KEYS_DIR, CHUNKS_DIR, CHUNK_SIZE,
        get_user_keys_dir, get_user_chunks_dir
    )
except ImportError:
    ...
    # Fonctions par défaut pour compatibilité
    def get_user_keys_dir(user_id):
        return KEYS_DIR
    def get_user_chunks_dir(user_id):
        return CHUNKS_DIR
...
class CryptoSystem:
    ...
    def __init__(self, user_id=None, keys_dir=None, chunks_dir=None, chunk_size=None):
        """
        Initialise le système cryptographique
        
        Args:
            user_id: ID de l'utilisateur (pour isolation des données)
            keys_dir: Répertoire des clés (optionnel, utilise user_id si fourni)
            chunks_dir: Répertoire des chunks (optionnel, utilise user_id si fourni)
            chunk_size: Taille des chunks (optionnel)
        """
        # Si user_id est fourni, utiliser les chemins par utilisateur
        if user_id is not None:
            keys_dir = keys_dir or get_user_keys_dir(user_id)
            chunks_dir = chunks_dir or get_user_chunks_dir(user_id)
